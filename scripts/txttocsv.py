...
for row in data:
    data_point = {
        'day': datetime.datetime.strptime(row[2:10], '%Y%m%d').day,
    ...
    key = (data_point['day'], data_point['stock'])
    if key not in dataset_aux:
        dataset_aux.add(key)
        dataset.append(data_point)

dataset = sorted(dataset, key=lambda x: (x['day'], x['stock']))

print(dataset[0])

csv_file = 'COTAHIST_2023.csv'
csv_headers = ['day', 'stock', 'open', 'high', 'low', 'close']
